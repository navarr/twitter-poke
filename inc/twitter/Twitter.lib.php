...
class TwitterAPI
{
	...
	{
		if(!$this->t->uid || $this->t_noauth || $this->noauth)
		{
...
class TWML
{
	...
	public function profile_pic($uid = NULL, $opts = NULL)
	{
		...
		{
			$img = "http://twivatar.koneko-chan.net/$sn/mini";
			$w = 24;
			$h = 24;
		}
		elseif ($opts["size"] == "thumb" || $opts["size"] == "normal")
		{
			$img = "http://twivatar.koneko-chan.net/$sn/normal";
			$w = 48;
			$h = 48;
		}
		elseif ($opts["size"] == "large" || $opts["size"] == "bigger")
		{
			$img = "http://twivatar.koneko-chan.net/$sn/bigger";
			$w = 73;
			$h = 73;
		}
		elseif ($opts["size"] == "full" || $opts["size"] == "original")
		{
			$img = "http://twivatar.koneko-chan.net/$sn/original";
		}
